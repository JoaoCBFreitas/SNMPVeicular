...
void init_requestMonitoringDataTable(void)
{
    ...
    req->reqID = 0;
    req->genericRequestID = 1;
    req->statisticsRequestID = 0;
    req->savingMode = 0;
    req->sampleFreq = 10;
    req->maxDelay = -10;
    ...
    index.len = 1;
    ctx = NULL;
    reqList = (requestMonitoringList *)malloc(sizeof(requestMonitoringList));
    reqList->capacity = 1;
    reqList->current = 1;
    reqList->requestList = malloc(sizeof(requestMonitoringStruct));
    reqList->requestList[0] = *req;
    /* Search for it first. */
...
void requestMonitoringDataTable_set_reserve1(netsnmp_request_group *rg)
{
    ...
    for (current = rg->list; current; current = current->next)
    {
        ...
        case COLUMN_GENERICREQUESTTYPEID:
            /** UNSIGNED32 = ASN_UNSIGNED */
            /* or possibly 'netsnmp_check_vb_int_range' */
            rc = netsnmp_check_vb_int(var);
            break;
...
void requestMonitoringDataTable_set_action(netsnmp_request_group *rg)
{
    ...
    for (current = rg->list; current; current = current->next)
    {
        ...
        case COLUMN_GENERICREQUESTTYPEID:
            /** UNSIGNED32 = ASN_UNSIGNED */
            row_ctx->genericRequestTypeID = *var->val.integer;
            break;
    ...
    netsnmp_table_request_info *table_info)
{
    ...
    case COLUMN_GENERICREQUESTTYPEID:
        /** UNSIGNED32 = ASN_UNSIGNED */
        snmp_set_var_typed_value(var, ASN_UNSIGNED,
                                 (char *)&context->genericRequestTypeID,
                                 sizeof(context->genericRequestTypeID));
        break;
...
requestMonitoringDataTable_context *requestMonitoringDataTable_create_row(netsnmp_index *hdr, requestMonitoringStruct *req)
{
    ...
    ctx->genericRequestTypeID = (long unsigned int)req->genericRequestID;
...
int requestMonitoringDataTable_row_copy(requestMonitoringDataTable_context *dst, requestMonitoringDataTable_context *src)
{
    ...
    dst->genericRequestTypeID = src->genericRequestTypeID;
...
requestMonitoringList *requestGet()
{
    return reqList;
}